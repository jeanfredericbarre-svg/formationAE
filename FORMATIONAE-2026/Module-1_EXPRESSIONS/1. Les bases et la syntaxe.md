...
-  Comme dit précédemment, une expression vit _à l'intérieur_ d'une propriété. En JS standard, tu écris un programme qui tourne une fois. Dans AE, chaque expression est **réévaluée à chaque frame**, comme si AE appelait ta fonction automatiquement 25 fois par seconde.
...
thisComp      // l'objet racine : la composition
  .layer(1)   // on descend dans un de ses enfants : le calque
    .position // on descend encore : une propriété de ce calque
      .value[1]  // on atteint la valeur finale
